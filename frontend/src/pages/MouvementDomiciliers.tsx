...
const handleConfirmRejetStatus = async (motif: string) => {
    ...
    if (selectedRowsForStatus.length === 1) {
        url += `/${selectedRowsForStatus[0].DOM_CODE}`; // route single
        body.DOM_MOTIF_REJET = motif; //  ajout du motif pour paiement unique
    } else {
        body = {
            ids: selectedRowsForStatus.map(r => r.DOM_CODE),
            DOM_MOTIF_REJET: motif //  ajout du motif pour paiement multiple
        };
    }
